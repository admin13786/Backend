...
interface RoundtableProps {
  ...
  /**
   * If provided, Roundtable will auto-open the text input and prefill it.
   * Used by the "讲解" button to insert the title into the chat input box.
   */
  readonly prefillInput?: string;
...
export function Roundtable({
  ...
  // Prefill input after navigating from a "讲解" card.
  // We apply once per distinct value to avoid overwriting user edits.
  const lastPrefilledValueRef = useRef<string | undefined>(undefined);
  useEffect(() => {
    if (!prefillInput) return;
    if (prefillInput === lastPrefilledValueRef.current) return;
    lastPrefilledValueRef.current = prefillInput;
    setInputValue(prefillInput);
    setIsVoiceOpen(false);
    setIsInputOpen(true); // textarea mounts -> autoFocus triggers
  }, [prefillInput]);
