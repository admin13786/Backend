...
#### Docker for local development (hot reload)

The default `docker compose` image runs `next build` + `node server.js`, so code edits on the host are not picked up. For **Docker + hot reload**, use the dev compose file:

```bash
# First time, or after Dockerfile.dev / dependency changes:
pnpm docker:dev:build
# Day to day:
pnpm docker:dev
```

Same as `docker compose -f docker-compose.dev.yml up` (add `--build` when needed).

The repo is **bind-mounted** into the container and runs `pnpm install` + `next dev`. If port **3000** is already used by the production container, change `ports` in `docker-compose.dev.yml` (e.g. `"3001:3000"`).

On the **first** start, `pnpm install` over the bind mount can take a long time; until it finishes, `localhost:3000` may show **connection reset / empty response**. Run `pnpm install` once on the **host** in this folder before `pnpm docker:dev` to speed things up. Follow logs with `docker logs -f openmaic-openmaic-dev-1` and open the app after you see **Ready**.
