...
#### Docker 里做开发（改代码不必每次全量 build）

生产用的 `docker compose` 镜像是先 `next build` 再跑 `node server.js`，改源码不会进容器。若希望**继续用 Docker、又要热更新**，请用开发编排：

```bash
# 首次或改了 Dockerfile.dev / 依赖时加 --build
pnpm docker:dev:build
# 之后日常只起服务即可（代码改在宿主机，容器里 next dev 会刷新）
pnpm docker:dev
```

等价命令：`docker compose -f docker-compose.dev.yml up`（需要重建时加 `--build`）。

说明：会把当前目录**挂载**进容器并在其中执行 `pnpm install` + `next dev`；与生产容器同时开时注意 **3000 端口冲突**（可改 `docker-compose.dev.yml` 里 `ports` 为 `"3001:3000"` 等）。

**第一次启动**时，`pnpm install` 在挂载目录上可能很慢，完成前浏览器访问 `localhost:3000` 会出现 **连接被重置 / 无响应**。可在项目目录先执行一次宿主机的 `pnpm install`，再 `pnpm docker:dev`，会快很多。用 `docker logs -f openmaic-openmaic-dev-1` 看到 **Ready** 后再打开页面。
