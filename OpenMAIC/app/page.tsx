...
function HomePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Handle prefill from URL, or Cookie set by POST /api/dialog-prefill
      let prefill = searchParams.get('prefill');
      if (!prefill) {
        const raw = document.cookie.match(/(?:^|; )openmaic_prefill=([^;]*)/)?.[1];
        if (raw) {
          try {
            prefill = decodeURIComponent(raw);
          } catch {
            prefill = raw;
          }
        }
      ...
      if (prefill) {
        updates.requirement = decodeURIComponent(prefill);
      }
  ...
  useEffect(() => {
    const applyEmbeddedTheme = (nextTheme: unknown) => {
    ...
    applyEmbeddedTheme(searchParams.get('theme'));

    const handleMessage = (event: MessageEvent) => {
      if (!event.data || typeof event.data !== 'object') return;
      if (event.data.type !== 'openmaic:set-theme') return;
