...
CLAUDE_IDLE_TIMEOUT_SECONDS = int(os.getenv("CLAUDE_IDLE_TIMEOUT_SECONDS", "900"))
    ...
    try:
        with sync_playwright() as playwright:
            ...
            try:
                ...
                if not has_visual_surface and body_text_length < 24:
    ...
    if claude_error or parsed_error or exit_code != 0:
        ...
        if runtime:
            upsert_claude_runtime_record(
                ...
                container_name=runtime["container_name"],
                status="running",
                last_error=claude_error or parsed_error or output or raw_output or f"Claude failed with exit code {exit_code}",
                last_used_at=utc_now(),
            )
        detail = claude_error or parsed_error or output or raw_output or f"Claude failed with exit code {exit_code}"
        yield sse_event(
            "error",
            {
                "message": detail,
